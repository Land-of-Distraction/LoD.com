...
ShopifyAPI.selectCallback = function(variant, selector) {
	...
	if ( selector.domIdPrefix.split('-').length > 1 ) {
		var selectorArray = selector.domIdPrefix.split('-'),
			$addToCart = $('#AddToCart-' + selectorArray[1]);
	} else {
		var $addToCart = $('#AddToCart');
	}

	var $productWrapper = $addToCart.parents('.product-info'), // Defined the Product Dom Scope
	...
	if (variant) {
		...
		if (variant.compare_at_price > variant.price) {
			// $comparePrice.html('compare at ' + Shopify.formatMoney(variant.compare_at_price, moneyFormat)).show();
			$comparePrice.html( Shopify.formatMoney(variant.compare_at_price, moneyFormat).replace(".00","") ).show();
		} else {
			$comparePrice.hide();
		}

		// 1R Yang : adding currency change trigger
		// mlvedaload();
...
var ajaxCart = (function(module, $) {
	...
	buildCart = function (cart) {
		...
		if (cart.item_count === 0) {
			$cartContainer
				.append('<div class="ajaxcart__empty">' + "Your bag is empty" + '</div>');
		...
		$.each(cart.items, function(index, cartItem) {
			...
			*/
			if (cartItem.image != null){
				var prodImg = cartItem.image.replace(/(\.[^.]*)$/, "_400x400$1").replace('http:', '');
			} else {
		...
		data = {
			...
			totalCartDiscount: cart.total_discount === 0 ? 0 : "\u003cp\u003eYou're saving\u003c\/p\u003e".replace('[savings]', Shopify.formatMoney(cart.total_discount, settings.moneyFormat)).replace(".00",""),
