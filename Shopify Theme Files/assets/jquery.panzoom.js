		...
		setMatrix: function(matrix, options) {
			...
			if (contain) {
				...
				if (this.$parent.css('textAlign') !== 'center' || $.css(this.elem, 'display') !== 'inline') {
					...
					// Windsor@Onerockwell: Only calculate Left width based on image width - screenW and we assign space from left to 0
					// spaceWLeft = spaceWRight = ((width - conWidth) / 2);
					spaceWLeft = 0;
					spaceWRight = ((width - conWidth) );
				}

				// Yang@Onerockwell Fix for contain invert vertical range 
				// var spaceHTop = ((height - conHeight) / 2) + dims.border.top;
				var spaceHTop = 0;
				var spaceHBottom = ((height - conHeight) / 2) - dims.border.top - dims.border.bottom;

				// Windsor@Onerockwell: X Axis
				if (contain === 'invert' || contain === 'automatic' && zoomAspectW < 1.01) {
					matrix[4] = Math.max(Math.min(matrix[4], spaceWLeft - dims.border.left), -spaceWRight);
				} else {
					matrix[4] = Math.min(Math.max(matrix[4], spaceWLeft), -spaceWRight);
				}
				// Windsor@Onerockwell: Y Axis
				if (contain === 'invert' || (contain === 'automatic' && zoomAspectH < 1.01)) {
					// Yang@Onerockwell Fix for contain invert vertical range 
					// matrix[5] = Math.max(Math.min(matrix[5], spaceHTop - dims.border.top), -spaceHBottom);
					matrix[5] = Math.max(Math.min(matrix[5], spaceHTop - dims.border.top), -spaceHBottom*2);
				} else {
